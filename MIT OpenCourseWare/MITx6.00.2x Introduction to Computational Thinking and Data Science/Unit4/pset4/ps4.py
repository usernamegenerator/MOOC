...
def generate_models(x, y, degs):
    ...
    """
    # TODO
    #polyfit is to find the curve over the data
    #such that the least squares is the minimuim
    #least squares:
    #lecture10-segment1.pdf, page 10
    returnArr = []
    for i in range(len(degs)):
        model = np.polyfit(x,y,degs[i])
...
def r_squared(y, estimated):
    ...
    meanError = error/len(y) # numerator 
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(len(models)):
        ...
        pylab.plot(x,estY,label='Fit of degree' + str(len(models[i])) + ', R2 = ' + str(round(error,5)))
        #pylab.plot(x,estY)
        pylab.legend(loc = 'best')
        #pylab.title('whatever')
...
# Each sample represents a year from 1961 to 2005 
# and the average annual temperature in Boston in that year
